...
export const ProfileLoanRecord:React.FC<ProfileLoanRecordProps> = ({record}) => {
    return (
        <div className="profile-record">
            ...
            <h4 className={record.status === "AVAILABLE" ? "status-returned" : "status-loaned"}>
                Status: {record.status === "AVAILABLE" ? "RETURNED" : "LOANED"}</h4>
            <p>Loan Date: {new Date(record.loanedDate).toDateString()}</p>
            <p>Return by Date: {new Date(record.dueDate).toDateString()}</p>
            {record.returnDate && <p>Date Returned: {new Date(record.returnDate).toDateString()}</p>}
        </div>
    )
}